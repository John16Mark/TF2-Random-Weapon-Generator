...
public class Weapon {
	...
	**************************************************************************************************/
	private void setTipoSyringeGun() {
		...
		syringeUberOnHit = new Stat(3,1,"On Hit: Up to +","% �berCharge added",42,true);															stats.add(syringeUberOnHit);	syringeUberOnHit.setNombre("\033[36msyringeUberOnHit\033[0m");
		speedBasedUber = new Stat(20, 5,"Up to +", "% faster movement speed based on �berCharge percentage when active", 25,
				"Up to +", "% slower movement speed based on �berCharge percentage when active", 15);												stats.add(speedBasedUber);		speedBasedUber.setNombre("\033[33mspeedBasedUber\033[0m");
		dmgBasedUber = new Stat(20, 5,"Up to +", "% damage bonus based on �berCharge percentage when active", 25,
				"Up to -", "% damage penalty based on �berCharge percentage when active",20);														stats.add(dmgBasedUber);		dmgBasedUber.setNombre("\033[33mdmgBasedUber\033[0m");
		fireSpdBasedUber = new Stat(20, 5,"Up to +", "% faster firing speed based on �berCharge percentage when active", 25,
				"Up to -", "% slower firing speed based on �berCharge percentage when active",20);													stats.add(fireSpdBasedUber);	fireSpdBasedUber.setNombre("\033[33mfireSpdBasedUber\033[0m");
	}
	...
	private void setTipoMedigun() {
		...
		uberRate = new Stat(65, 5, "+", "% �bercharge rate", 30, "-", "% �bercharge rate", 20);				stats.add(uberRate);			uberRate.setNombre("\033[33muberRate\033[0m");
	...
	private void setTipoBonesaw() {
		onHitUber = new Stat(45, 5, "On Hit: +" ,"% �berCharge added", 60, true);																	stats.add(onHitUber);			onHitUber.setNombre("\033[33monHitUber\033[0m");
	...
	public void generarArma() {
		...
		if(isBanner) {
			while(banner.size() < 4 && puntuacionBanner <= maxPuntuacionBanner) {
				...
				// Si la puntuaci�n ya super� el promedio, puede seguir o no de manera aleatoria
				...
				// Analiza todos los stats y si cabe dentro de la puntuaci�n, lo toma
				do {
					...
					// Buscar el stat que tiene la puntuaci�n menor e ir acumulando su �ndice
					if(statAux.getPuntajeMenor(true) < bannerBuffs.get(indexOfMin).getPuntajeMenor(true)) {
						indexOfMin = aux;
					}
					// Buscar el stat que tiene la puntuaci�n mayor e ir acumulando su �ndice
					if(statAux.getPeso() > bannerBuffs.get(indexOfMax).getPeso()) {
						indexOfMax = aux;
					}
					// Si el stat es el �ltimo y es muy chico, buscar otro. Si es muy grande buscar otro
				...
				// Si el stat de menor peso sobrepasa el m�ximo, ya no a�ade m�s stats
				...
				// A�adir el stat a la lista de stats
				System.out.print("\n\n   "+statAux.getTexto()+"\n");
				banner.add(statAux);
		...
		System.out.print("\nPuntuaci�n banner = "+puntuacionBanner);
		...
		if(power != 5) {
			 ...
			 **********************************************************************************************************************************/
				...
				while(positivos.size() < maxStatsPositivos && statsPositivosDisponibles != 0) {
					...
					// Si no se encontr� un buen stat
					if(correcto == false) {
						// Si el m�s chico es muy grande
						if(
		...
		System.out.print("\n\n PUNTUACI�N POSITIVA: "+ puntuacionPositiva);
		System.out.print("\n PUNTUACI�N NEGATIVA: "+ puntuacionNegativa);
		System.out.print("\n PUNTUACI�N BANNER: "+ puntuacionBanner);
	}
